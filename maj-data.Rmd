...
etab <- bind_rows(
  etab.21 %>% filter(UAI %in% univ21$UAI) %>% mutate(Groupe = "Université"),
  etab.21 %>% filter(UAI %in% nonuniv.complet$UAI) %>% mutate(Groupe = "Autre"),
  etab.20 %>% filter(UAI %in% nonuniv.complet.etab20$UAI) %>% mutate(Groupe = "Autre")
  )
