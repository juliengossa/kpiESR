...
res <- fin %>%
  ...
  mutate(
    Ressources.formation = rowSums(na.rm = TRUE, select(.,
         Droits.d.inscription,
         Formation.continue..diplômes.propres.et.VAE, 
         Taxe.d.apprentissage)),
    Ressources.recherche = rowSums(na.rm = TRUE, select(.,
         Valorisation,
         ANR.hors.investissements.d.avenir, 
         ANR.investissements.d.avenir,
         Contrats.et.prestations.de.recherche.hors.ANR)),
    Ressources.autres = rowSums(na.rm = TRUE, select(.,
         Subventions.de.la.région, 
         Subventions.Union.Européenne, 
         Autres.ressources.propres,
         Autres.Subventions)),
...
res %>%
  ...
  select(Type, SCSP, Ressources.formation, Ressources.recherche, Ressources.autres) %>%
...
res %>%
  select(exercice, SCSP, Ressources.formation, Ressources.recherche, Ressources.autres) %>%
...
res %>%
  ...
  summarise(SCSP = sum(SCSP)) %>%
  group_by(Type) %>%
  mutate(evolution.SCSP = SCSP / first(SCSP) - 1) %>%
