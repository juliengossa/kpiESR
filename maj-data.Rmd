...
```{r uai.manquants}
etab.21 %>% filter(is.na(UAI)) %>% kableExtra::kable(format="pipe")
```
...
correspondances.uai %>% kableExtra::kable(format="pipe")
```
...
nonuniv.complet %>% kableExtra::kable(format="pipe")
```
...
wikidata.fix <- bind_rows(
 ...
 wikidata.fix %>% kableExtra::kable(format="pipe")
```
...
```{r sigles.wikidata.print}
alias.wikidata %>% kableExtra::kable(format="pipe")
```
