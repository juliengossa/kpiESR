...
univ21 <- etab.21 %>% select(UAI, etab.21 = Etablissement) %>% filter(str_detect(etab.21,"niversité")) %>%
  left_join(ens %>% fix_uai(correspondances.uai) %>% group_by(UAI) %>% summarise(ens = paste(range(Rentrée), collapse = "-")))  %>%
  left_join(etu %>% fix_uai(correspondances.uai) %>% group_by(UAI) %>% summarise(etu = paste(range(Rentrée), collapse = "-"))) %>%
  left_join(fin %>% fix_uai(correspondances.uai) %>% group_by(UAI) %>% summarise(fin = paste(range(Rentrée), collapse = "-")))
